...
export default function UsersPage() {
  ...
  const applyFilter = (all: User[], role: string) => {
    const displayed = role === "all" ? all : all.filter((u) => u.role === role);
    setUsers(displayed);
    setCounts({
      all: all.length,
      manager: 0,
  ...
  useEffect(() => {
    ...
    searchRef.current = setTimeout(() => fetchUsers(search), 350);
    return () => {
      if (searchRef.current) clearTimeout(searchRef.current);
    };
  }, [search]);
  ...
  const handleRoleFilter = (r: string) => {
    setRoleFilter(r);
    applyFilter(allUsersRef.current, r);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (formMode === "create") {
        ...
        await userService.update(editUser.userId, {
              ...
              {users.map((u) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border w-fit ${cfg.color}`}
                      >
                        <cfg.icon size={10} />
                        {cfg.label}
                      </span>
